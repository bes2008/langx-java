 ...
 */
public class SM4BlockCipher implements BlockCipher
{
    ...
    static {
        SM4BlockCipher.FK = new int[] { -1548633402, 1453994832, 1736282519, -1301273892 };
        SM4BlockCipher.BLOCK_SIZE = 16;
    }

    public SM4BlockCipher() {
    ...
    public void init(final boolean forEncryption, final CipherParameters cipherParameters) throws IllegalArgumentException {
        ...
        if (key.length != SM4BlockCipher.BLOCK_SIZE) {
    ...
    public int getBlockSize() {
        return SM4BlockCipher.BLOCK_SIZE;
    }
    ...
    private int[] keyExpension(final int[] array) {
        ...
        for (int i = 0; i < 4; ++i) {
            array2[i] = (array[i] ^ SM4BlockCipher.FK[i]);
        }
