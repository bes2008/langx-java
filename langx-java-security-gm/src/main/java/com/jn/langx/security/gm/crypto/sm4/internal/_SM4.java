...
class _SM4
{
    ...
    static {
        _SM4.FK = new int[] { -1548633402, 1453994832, 1736282519, -1301273892 };
        _SM4.BLOCK_SIZE = 16;
    }
    ...
    public void setKey(final byte[] array) throws CryptoException {
        if (array.length != _SM4.BLOCK_SIZE) {
    ...
    public int encrypt(final byte[] array, final int n, final int n2, final byte[] array2, final int n3) throws IllegalBlockSizeException {
        if (n2 % _SM4.BLOCK_SIZE != 0) {
            throw new IllegalBlockSizeException();
        }
        final int n4 = n2 / _SM4.BLOCK_SIZE;
        switch (this.mode) {
            case 0: {
                for (int i = 0; i < n4; ++i) {
                    this.encrytBlock(array, n + _SM4.BLOCK_SIZE * i, array2, n3 + _SM4.BLOCK_SIZE * i);
                }
                break;
            }
            case 1: {
                if (this.opMode == 1) {
                    ...
                    for (int j = 0; j < n4; ++j) {
                        for (int k = 0; k < _SM4.BLOCK_SIZE; ++k) {
                            final byte[] array3 = iv;
                            final int n5 = k;
                            array3[n5] ^= array[n + _SM4.BLOCK_SIZE * j + k];
                        }
                        this.encrytBlock(iv, 0, array2, n3 + _SM4.BLOCK_SIZE * j);
                        for (int l = 0; l < _SM4.BLOCK_SIZE; ++l) {
                            iv[l] = array2[n3 + _SM4.BLOCK_SIZE * j + l];
                        }
                    }
                    break;
                }
                final byte[] iv2 = this.iv;
                final byte[] array4 = new byte[_SM4.BLOCK_SIZE];
                for (int n6 = 0; n6 < n4; ++n6) {
                    final int n7 = n + _SM4.BLOCK_SIZE * n6;
                    final int n8 = n3 + _SM4.BLOCK_SIZE * n6;
                    System.arraycopy(array, n7, array4, 0, _SM4.BLOCK_SIZE);
                    this.encrytBlock(array, n7, array2, n8);
                    for (int n9 = 0; n9 < _SM4.BLOCK_SIZE; ++n9) {
                        final int n10 = n3 + _SM4.BLOCK_SIZE * n6 + n9;
                        array2[n10] ^= iv2[n9];
                    }
                    System.arraycopy(array4, 0, iv2, 0, _SM4.BLOCK_SIZE);
                }
                break;
            }
            default: {
                for (int n11 = 0; n11 < n4; ++n11) {
                    this.encrytBlock(array, n + _SM4.BLOCK_SIZE * n11, array2, n3 + _SM4.BLOCK_SIZE * n11);
                }
                break;
            }
        }
