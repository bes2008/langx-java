...
import com.jn.langx.security.crypto.digest.internal.impl._Blake2bDigest;
...
class Argon2BytesGenerator {
    ...
    private void hash(byte[] input, byte[] out, int outOff, int outLen) {
        ...
        Bytes.intToLittleEndian(outLen, outLenBytes, 0);

        int blake2bLength = 64;

        if (outLen <= blake2bLength) {
            _Blake2bDigest blake = new _Blake2bDigest(outLen * 8);

            blake.update(outLenBytes, 0, outLenBytes.length);
            blake.update(input, 0, input.length);
            blake.doFinal(out, outOff);
        } else {
            _Blake2bDigest digest = new _Blake2bDigest(blake2bLength * 8);
            ...
            /* Vr+1 */
            digest = new _Blake2bDigest(lastLength * 8);
...
//    }
    ...
    private void initialize(byte[] tmpBlockBytes, byte[] password, int outputLength) {
        ...
        _Blake2bDigest blake = new _Blake2bDigest(ARGON2_PREHASH_DIGEST_LENGTH * 8);
        ...
        Bytes.intToLittleEndian(values, tmpBlockBytes, 0);
    ...
    private static void addByteString(byte[] tmpBlockBytes, Digest digest, byte[] octets) {
        ...
        Bytes.intToLittleEndian(octets.length, tmpBlockBytes, 0);
...
//        Pack.intToLittleEndian(1, initialHashWithOnes, ARGON2_PREHASH_DIGEST_LENGTH);
        ...
        for (int i = 0; i < parameters.getLanes(); i++) {
            Bytes.intToLittleEndian(i, initialHashWithZeros, ARGON2_PREHASH_DIGEST_LENGTH + 4);
            Bytes.intToLittleEndian(i, initialHashWithOnes, ARGON2_PREHASH_DIGEST_LENGTH + 4);
    ...
    private static class Block {
        ...
        void fromBytes(byte[] input) {
            ...
            Bytes.littleEndianToLong(input, 0, v);
        }
        ...
        void toBytes(byte[] output) {
            ...
            Bytes.longToLittleEndian(v, output, 0);
        }
