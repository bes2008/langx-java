...
import com.jn.langx.security.crypto.pbe.pbkdf.DerivedPBEKeyGenerator;
import com.jn.langx.util.Ints;
...
public class SCrypt {
    ...
    private static byte[] MFcrypt(byte[] P, byte[] S, int N, int r, int p, int dkLen) {
        ...
        try {
            ...
            Bytes.littleEndianToInt(bytes, 0, B);
            ...
            Bytes.intToLittleEndian(B, bytes, 0);

            return SingleIterationPBKDF2(P, bytes, dkLen);
        } finally {
            PrimitiveArrays.clear(bytes);
            PrimitiveArrays.clear(B);
        }
    }

    private static byte[] SingleIterationPBKDF2(byte[] P, byte[] S, int dkLen) {
        DerivedPBEKeyGenerator pGen = new PKCS5S2ParametersGenerator(new _SHA256Digest());
        pGen.init(P, S, 1);
    ...
    private static void SMix(int[] B, int BOff, int N, int d, int r) {
        int powN = Ints.numberOfTrailingZeros(N);
    ...
    private static void ClearAll(int[][] arrays) {
        for (int i = 0; i < arrays.length; ++i) {
            PrimitiveArrays.clear(arrays[i]);
        }
    }
