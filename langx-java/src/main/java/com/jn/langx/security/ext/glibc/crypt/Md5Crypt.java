...
class Md5Crypt {
     ...
     * <p>
     * See {@link GlibcCrypt#crypt(String, String)} for details. We use {@link SecureRandom} for seed generation by
     * default.
     * </p>
     *
     * @param keyBytes plaintext string to hash. Each array element is set to {@code 0} before returning.
     ...
     * <p>
     * See {@link GlibcCrypt#crypt(String, String)} or {@link #apr1Crypt(String, String)} for details. We use
     ...
     * Generates a libc6 crypt() "$1$" or Apache htpasswd "$apr1$" hash value.
     * <p>
     * See {@link GlibcCrypt#crypt(String, String)} or {@link #apr1Crypt(String, String)} for details.
     * </p>
     *
     * @param keyBytes plaintext string to hash. Each array element is set to {@code 0} before returning.
     * @param salt     real salt value without prefix or "rounds=". The salt may be null, in which case a salt
    ...
    public static String md5Crypt(final byte[] keyBytes, final String salt, final String prefix, final Random random) {
        ...
        if (salt == null) {
            ...
            Objs.requireNonNull(prefix, "prefix");
            if (prefix.length() < 3) {
        ...
        final MessageDigest ctx = MessageDigests.getDigest(JCAEStandardName.MD5.getName());
         ...
         */
        MessageDigest ctx1 = MessageDigests.getDigest(JCAEStandardName.MD5.getName());
        ...
        for (int i = 0; i < ROUNDS; i++) {
            ctx1 = MessageDigests.getDigest(JCAEStandardName.MD5.getName());
            if ((i & 1) != 0) {
                ctx1.update(keyBytes);
            } else {
