...
public class Ciphers extends Securitys {
    ...
    public static Cipher createCipher(@NonNull String algorithmTransformation, @Nullable Provider provider, int operateMode, Key key, SecureRandom secureRandom) {
        try {
            ...
            checkKeyLength(cipher.getProvider().getName(), key);
    ...
    private static void checkKeyLength(String providerName,Key key) throws NoSuchAlgorithmException{
        if(Strings.startsWith(providerName, "sun", true)){
            int maxAllowedKeyLength = Cipher.getMaxAllowedKeyLength(key.getAlgorithm());
