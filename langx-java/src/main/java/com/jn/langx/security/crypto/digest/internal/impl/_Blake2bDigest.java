...
import com.jn.langx.util.Longs;
 ...
 */
public class _Blake2bDigest implements Digest {
    ...
    public _Blake2bDigest() {
        this(512);
    }

    public _Blake2bDigest(_Blake2bDigest digest) {
        this.bufferPos = digest.bufferPos;
        this.buffer = PrimitiveArrays.clone(digest.buffer);
        this.keyLength = digest.keyLength;
        this.key = PrimitiveArrays.clone(digest.key);
        this.digestLength = digest.digestLength;
        this.chainValue = PrimitiveArrays.clone(digest.chainValue);
        this.personalization = PrimitiveArrays.clone(digest.personalization);
        this.salt = PrimitiveArrays.clone(digest.salt);
        this.t0 = digest.t0;
        this.t1 = digest.t1;
        this.f0 = digest.f0;
    }
     ...
     */
    public _Blake2bDigest(int digestSize) {
     ...
     */
    public _Blake2bDigest(byte[] key) {
        buffer = new byte[BLOCK_LENGTH_BYTES];
        if (key != null) {
     ...
     */
    public _Blake2bDigest(byte[] key, int digestLength, byte[] salt, byte[] personalization) {
    ...
    private void init() {
        if (chainValue == null) {
            ...
            if (salt != null) {
                chainValue[4] ^= Bytes.littleEndianToLong(salt, 0);
                chainValue[5] ^= Bytes.littleEndianToLong(salt, 8);
            }

            chainValue[6] = blake2b_IV[6];
            chainValue[7] = blake2b_IV[7];
            if (personalization != null) {
                chainValue[6] ^= Bytes.littleEndianToLong(personalization, 0);
                chainValue[7] ^= Bytes.littleEndianToLong(personalization, 8);
            }
        }
    }
    ...
    public int doFinal(byte[] out, int outOffset) {
        ...
        for (int i = 0; i < chainValue.length && (i * 8 < digestLength); i++) {
            byte[] bytes = Bytes.longToLittleEndian(chainValue[i]);
    ...
    private void compress(byte[] message, int messagePos) {
        ...
        long[] m = new long[16];
        for (int j = 0; j < 16; j++) {
            m[j] = Bytes.littleEndianToLong(message, messagePos + j * 8);
        }
