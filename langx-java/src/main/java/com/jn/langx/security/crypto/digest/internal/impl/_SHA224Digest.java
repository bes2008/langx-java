 ...
 * </pre>
 */
public final class _SHA224Digest extends GeneralDigest {
    ...
    public _SHA224Digest(byte[] encodedState) {
        ...
        H2 = Bytes.bigEndianToInt(encodedState, 20);
        H3 = Bytes.bigEndianToInt(encodedState, 24);
        H4 = Bytes.bigEndianToInt(encodedState, 28);
        H5 = Bytes.bigEndianToInt(encodedState, 32);
        H6 = Bytes.bigEndianToInt(encodedState, 36);
        H7 = Bytes.bigEndianToInt(encodedState, 40);
        H8 = Bytes.bigEndianToInt(encodedState, 44);

        xOff = Bytes.bigEndianToInt(encodedState, 48);
        for (int i = 0; i != xOff; i++) {
            X[i] = Bytes.bigEndianToInt(encodedState, 52 + (i * 4));
        }
    }
...
//        X[xOff] = Pack.bigEndianToInt(in, inOff);
    ...
    public int doFinal(
        ...
        Bytes.intToBigEndian(H2, out, outOff + 4);
        Bytes.intToBigEndian(H3, out, outOff + 8);
        Bytes.intToBigEndian(H4, out, outOff + 12);
    ...
    public byte[] getEncodedState() {
        ...
        Bytes.intToBigEndian(H1, state, 16);
        Bytes.intToBigEndian(H2, state, 20);
        Bytes.intToBigEndian(H3, state, 24);
        Bytes.intToBigEndian(H4, state, 28);
        Bytes.intToBigEndian(H5, state, 32);
        Bytes.intToBigEndian(H6, state, 36);
        Bytes.intToBigEndian(H7, state, 40);
        Bytes.intToBigEndian(H8, state, 44);
        Bytes.intToBigEndian(xOff, state, 48);

        for (int i = 0; i != xOff; i++) {
            Bytes.intToBigEndian(X[i], state, 52 + (i * 4));
        }
