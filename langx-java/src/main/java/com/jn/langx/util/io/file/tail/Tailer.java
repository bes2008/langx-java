...
public class Tailer implements Runnable {
    ...
    public Tailer(final File file, final TailerListener listener, final long delayMillis, final boolean end) {
        this(file, listener, delayMillis, end, IOs.DEFAULT_BUFFER_SIZE);
    }
    ...
    public Tailer(final File file, final TailerListener listener, final long delayMillis, final boolean end,
        ...
        this(file, listener, delayMillis, end, reOpen, IOs.DEFAULT_BUFFER_SIZE);
    }
    ...
    public static Tailer create(final File file, final TailerListener listener, final long delayMillis,
                                final boolean end) {
        return create(file, listener, delayMillis, end, IOs.DEFAULT_BUFFER_SIZE);
    }
    ...
    public static Tailer create(final File file, final TailerListener listener, final long delayMillis,
                                final boolean end, final boolean reOpen) {
        return create(file, listener, delayMillis, end, reOpen, IOs.DEFAULT_BUFFER_SIZE);
    }
    ...
    public void run() {
        ...
        try {
            ...
            while (getRun()) {
                final boolean newer = Files.isFileNewer(file, last); // IO-279, must be done first
                ...
                if (length > position) {
                    ...
                    last = Files.getLastModified(file);
                } else if (newer) {
                    /*
    ...
    private long readLines(final RandomAccessFile reader) throws IOException {
        ...
        try  {
            ...
            while (getRun() && ((num = reader.read(inbuf)) != IOs.EOF)) {
                for (int i = 0; i < num; i++) {
