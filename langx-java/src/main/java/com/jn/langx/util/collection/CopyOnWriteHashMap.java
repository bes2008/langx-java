...
import com.jn.langx.util.struct.mutable.MutableValueInt;
...
public final class CopyOnWriteHashMap<K, V> extends AbstractMap<K, V> {
    ...
    private static class Leaf<K, V> extends Node<K, V> {
        ...
        void visit(Deque<Map.Entry<K, V>> entries, Deque<Node<K, V>> nodes) {
            for (int i = 0; i < keys.length; ++i) {
                entries.add(new AbstractMap.SimpleImmutableEntry<K, V>(keys[i], values[i]));
            }
        }
        ...
        void visit(Deque<Map.Entry<K, V>> entries, Deque<Node<K, V>> nodes) {
            for (int i = 0; i < keys.length; ++i) {
                ...
                if (sub instanceof Node) {
                    @SuppressWarnings("unchecked") final Node<K, V> subNode = (Node<K, V>) sub;
                    assert keys[i] == null;
                    nodes.add(subNode);
                } else {
                    @SuppressWarnings("unchecked") final V value = (V) sub;
                    entries.add(new AbstractMap.SimpleImmutableEntry<K, V>(keys[i], value));
                }
            }
        }
        ...
        V get(Object key, int hash) {
            ...
            if (sub instanceof Node) {
                ...
                if (keys[slot].equals(key)) {
                    @SuppressWarnings("unchecked") final V v = (V) sub;
                    return v;
                } else {
    ...
    private static class EntryIterator<K, V> implements Iterator<Map.Entry<K, V>> {
        ...
        EntryIterator(Node<K, V> node) {
            entries = new ArrayDeque<Map.Entry<K, V>>();
            nodes = new ArrayDeque<Node<K, V>>();
    ...
    public CopyOnWriteHashMap<K, V> copyAndRemove(Object key) {
        ...
        if (root == newRoot) {
            ...
            return new CopyOnWriteHashMap<K, V>(newRoot, size - 1);
        }
    }
