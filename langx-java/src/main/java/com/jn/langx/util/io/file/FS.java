...
public enum FS {
     ...
     */
    private static final FS CURRENT = current();
    ...
    private static boolean getOsMatchesName(final String osNamePrefix) {
        return isOsNameMatch(SystemPropertys.get("os.name"), osNamePrefix);
    }
     ...
     * @since 5.2.9 updated to behave more like {@link String}
     */
    ...
    FS(final int blockSize,
       ...
       final char nameSeparator) {
        ...
        this.reservedFileNames = Objs.requireNonNull(reservedFileNames, "reservedFileNames");
        ...
        this.nameSeparatorOther = Filenames.flipSeparator(nameSeparator);
    }
    ...
    public boolean isLegalFileName(final CharSequence candidate) {
        ...
        return Collects.<Character>noneMatch(new Predicate<Character>() {
            @Override
            public boolean test(Character c) {
                return isIllegalFileNameChar(c);
            }
        });
    }
    ...
    public String normalizeSeparators(final String path) {
        return Strings.replaceChars(path, nameSeparatorOther, nameSeparator);
    }
    ...
    public String toLegalFileName(final String candidate, final char replacement) {
        ...
        char[] chars = Strings.chars(truncated);
        char[] chars1 = new char[chars.length];
        for (int i = 0; i < chars.length; i++) {
            char c = chars[i];
            chars1[i] = isIllegalFileNameChar(c) ? replacement : c;
        }
