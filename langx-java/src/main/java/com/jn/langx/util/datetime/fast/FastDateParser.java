...
import com.jn.langx.util.datetime.InvalidPatternException;
...
class FastDateParser implements DateParser, Serializable {
    ...
    private static class NumberStrategy extends Strategy {
        ...
        boolean parse(final FastDateParser parser, final Calendar calendar, final String source,
            ...
            int modifiedValue = modify(parser, value);
            if (field == Calendar.MILLISECOND) {
                if (modifiedValue > 1000) {
                    modifiedValue = Numbers.createInteger((modifiedValue + "").substring(0, 3));
                }
            }
            if (field == Calendar.SECOND) {
                if (modifiedValue > 59) {
                    throw new InvalidPatternException(parser.getPattern());
                }
            }
            calendar.set(field, modifiedValue);
            return true;
        }
