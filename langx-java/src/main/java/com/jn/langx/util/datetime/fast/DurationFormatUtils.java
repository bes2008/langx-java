...
class DurationFormatUtils {
    ...
    public static String formatDuration(final long durationMillis, final String format, final boolean padWithZeros) {
        Preconditions.checkTrue( durationMillis >=0, "durationMillis must not be negative");
    ...
    public static String formatDurationWords(
        ...
        if (suppressLeadingZeroElements) {
            ...
            duration = " " + duration;
            String tmp = Strings.replaceOnce(duration, " 0 days", Strings.EMPTY);
            if (tmp.length() != duration.length()) {
                duration = tmp;
                tmp = Strings.replaceOnce(duration, " 0 hours", Strings.EMPTY);
                if (tmp.length() != duration.length()) {
                    duration = tmp;
        ...
        if (suppressTrailingZeroElements) {
            String tmp = Strings.replaceOnce(duration, " 0 seconds", Strings.EMPTY);
            if (tmp.length() != duration.length()) {
                duration = tmp;
                tmp = Strings.replaceOnce(duration, " 0 minutes", Strings.EMPTY);
                if (tmp.length() != duration.length()) {
                    ...
                    tmp = Strings.replaceOnce(duration, " 0 hours", Strings.EMPTY);
                    if (tmp.length() != duration.length()) {
                        duration = Strings.replaceOnce(tmp, " 0 days", Strings.EMPTY);
                    }
                }
            }
        }
    ...
    public static String formatPeriod(final long startMillis, final long endMillis, final String format, final boolean padWithZeros,
        ...
        Preconditions.checkTrue(startMillis <= endMillis, "startMillis must not be greater than endMillis");
    ...
    private static String paddedValue(final long value, final boolean padWithZeros, final int count) {
        ...
        return padWithZeros ? Strings.leftPad(longString, count, '0') : longString;
    }
